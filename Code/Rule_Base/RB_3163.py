...
for i,j in zip(files, file_name):
    ...
    # 選基金策略
    ret = data.pct_change()
    l2 = int(len(data.columns)/2)
    l3 = int(len(data.columns)/3)

    rule1 = ret.iloc[-720:].mean().nlargest(l2).index
    rule2 = ret.iloc[-240:].mean().nlargest(l2).index
    rule3 = ret.iloc[-120:].mean().nlargest(l2).index
    # rule4 = ret.iloc[-60:].mean().nlargest(l3).index

    # 篩選出符合條件的基金
    filt = [i for i in ret if i in rule1]
    filt = [i for i in filt if i in rule2]
...
workbook.save(path + 'RB_result/RuleBase_3163_result.xlsx')
workbook.close()
#%%
...
em_equity = pd.read_csv(path + 'em_equity.csv', parse_dates=True, index_col='Datetime')
US_hy_bond  = pd.read_csv(path + 'US_hy_bond.csv', parse_dates=True, index_col='Datetime')
us_eqity = pd.read_csv(path + 'us_eqity.csv', parse_dates=True, index_col='Datetime')
glo_ig_bond = pd.read_csv(path + 'glo_ig_bond.csv', parse_dates=True, index_col='Datetime')

def invest_funds(df):
    # 選基金策略
    ret = df.pct_change()
    l2 = int(len(df.columns)/2)
    rule1 = ret.iloc[-720:].mean().nlargest(l2).index
    rule2 = ret.iloc[-240:].mean().nlargest(l2).index
    rule3 = ret.iloc[-120:].mean().nlargest(l2).index
...
# em_equity
invest_funds(df = em_equity)

# US_hy_bond
invest_funds(df = US_hy_bond)

# us_eqity
invest_funds(df = us_eqity)

# glo_ig_bond
invest_funds(df = glo_ig_bond)
